...
def printMap(V):
    ...
    for starch in starchers:
        ...
        if (time.time() - starch.time >= 10):
            map_matrix[a][b] = archColor(starch) + Fore.BLACK + 'ST'
            map_matrix[a+1][b] = archColor(starch) + Fore.BLACK + 'CH'
            map_matrix[a][b+1] = archColor(starch) + Fore.BLACK + 'AR'
            map_matrix[a+1][b+1] = archColor(starch) + Fore.BLACK + 'ER'
    ...
    for h in healers:
        a= 2*h.position[0]
        b = 2*h.position[1]
        map_matrix[a][b] = blColor(h) + Fore.BLACK + 'HE'
        map_matrix[a+1][b] = blColor(h) + Fore.BLACK + 'ER'
        map_matrix[a][b+1] = blColor(h) + Fore.BLACK + 'AL'
        map_matrix[a+1][b+1] = blColor(h) + Fore.BLACK + '  '
