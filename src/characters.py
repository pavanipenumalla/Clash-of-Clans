...
class Healer:
    ...
    def move(self, x,y, troop):
        if(self.alive == False):
            return
        # vmap = V.map
        r = abs(x - self.position[0])
        c = abs(y - self.position[1])
    ...
    def heal(self,t):
        if not self.alive:
            return
        troops = [troop for troop in (barbarians + archers + dragons + balloons + starchers) if troop.alive]
        for troop in troops:
            if ((troop.position[0] - t.position[0])**2 + (troop.position[1] - t.position[1])**2) <= self.radius**2:
                if troop.alive:
...
def spawnStarcher(pos):
    if(pt.troop_limit['starcher'] <= troops_spawned['starcher']):
        return

    # convert tuple to list
    pos = list(pos)
    starcher = StArcher(pos)
    troops_spawned['starcher'] += 1
    starchers.append(starcher)
...
def move_starchers(V,type):
    if(type == 1):
        for archer in starchers:
            if(archer.alive == False):
                continue
            if archer.target != None:
                if(V.map[archer.target[0]][archer.target[1]] == pt.BLANK):
...
def move_healers(V):
    for healer in healers:
        ...
        if(closest_troop == None):
            continue
        healer.move(closest_troop.position[0], closest_troop.position[1], closest_troop )
...
def search_for_closest_troop(pos):
    troops = barbarians + archers + starchers + balloons + dragons
    closest_troop = None
    closest_dist = float('inf')
    for troop in troops:
        dx = troop.position[0] - pos[0]
        dy = troop.position[1] - pos[1]
        dist = dx ** 2 + dy ** 2
        if dist < closest_dist:
            closest_dist = dist
            closest_troop = troop
